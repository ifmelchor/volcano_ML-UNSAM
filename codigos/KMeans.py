...
df = df.drop(["CentroidPSD"], axis=1)
# df = df.drop(["HjorthComplex_d5_t1"], axis=1)
# df = df.drop(["HjorthMobil_d5_t1"], axis=1)
...
print(' init, n_cluster, silhouette')
print('_____________________________')
for init in ["k-means++", "random"]:
    for n_clusters in [2, 3, 4, 5, 6]:
        km = KMeans(n_clusters=n_clusters, init=init, random_state=0)
        cluster_labels = km.fit_predict(X)
        silhouette_avg = silhouette_score(X, cluster_labels)
        print(f'{init:>9}', f'{n_clusters:<2}', f'{silhouette_avg:<10.2f}')
print('')
...
df = pd.DataFrame({'LP_labels': labels}, columns=['LP_labels'])
df.to_json(file_out)
